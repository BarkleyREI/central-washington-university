	...
	init() {
		if (this.elem.$chat.length > 0) {
			...
			this.elem.$chat.find('.chat__button').on('click', function() {
				...
				chat.elem.$chat.find('.chat__button span').text(function() {
					return(this.textContent === 'Ask Here') ? 'Close Chat' : 'Ask Here';
				});
			...
			this.elem.$open.on('click', function(e) {
				...
				chat.elem.$chat.find('.chat__button span').text('Close Chat');
